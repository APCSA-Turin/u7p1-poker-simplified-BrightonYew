...
public class Player{
    ...
    public void sortAllCards(){ //originally SortCards
        //allCards.sort((c1, c2) -> Integer.compare(Utility.getRankValue(c1.getRank()),Utility.getRankValue(c2.getRank())));
        boolean notincorrectplace = true;
        int currentNum = 0;
        int currentIndex = 0;
        Card tempCard = null;


        for (int i = 0; i < allCards.size(); i++) { //goes through entire list
            currentNum = Utility.getRankValue(allCards.get(i).getRank());
            notincorrectplace = true;
            currentIndex = i - 1;

            while (notincorrectplace && currentIndex > -1) {

                if (currentNum < Utility.getRankValue(allCards.get(currentIndex).getRank())) { //swap
                    tempCard = allCards.get(currentIndex);
                    allCards.set(currentIndex, allCards.get(currentIndex + 1));
                    allCards.set(currentIndex + 1, tempCard);
                } else {
                    notincorrectplace = false;
                }
                currentIndex--;
            }
            }
